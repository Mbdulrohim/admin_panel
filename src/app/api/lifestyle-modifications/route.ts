...
type LifestyleModificationUpdateInput = Partial<{
  title: string;
  content: string;
  image_url: string;
...
// GET: Fetch all lifestyle modifications
export async function GET(request: Request) {
  try {
    const modifications = await prisma.lifestyle_modifications.findMany();

    // Generate signed URLs for image access
    const modificationsWithSignedUrls = modifications.map((modification) => ({
      ...modification,
      image_url: modification.image_url
        ? s3.getSignedUrl("getObject", {
            Bucket: process.env.AWS_S3_BUCKET_NAME,
            Key: modification.image_url.split("/").slice(-2).join("/"),
            Expires: 60 * 60, // 1 hour expiry
          })
        : null,
    }));

    return NextResponse.json(modificationsWithSignedUrls, { status: 200 });
  } catch (error) {
...
// POST: Create a new lifestyle modification
export async function POST(request: Request) {
  try {
    ...
    const fileKey = `lifestyle-modifications/${Date.now()}-${file.name}`;
    ...
    const newModification = await prisma.lifestyle_modifications.create({
      data: {
        title,
        content,
        status,
    ...
    return NextResponse.json(newModification, { status: 200 });
  } catch (error) {
...
// PUT: Update an existing lifestyle modification
export async function PUT(request: Request) {
  try {
    const contentType = request.headers.get("Content-Type");
    const updateData: LifestyleModificationUpdateInput = {};
    ...
    const updatedModification = await prisma.lifestyle_modifications.update({
      where: { id: Number(id) },
      data: updateData,
    });

    return NextResponse.json(updatedModification, { status: 200 });
  } catch (error) {
...
// DELETE: Delete a lifestyle modification
export async function DELETE(request: Request) {
  try {
    ...
    await prisma.lifestyle_modifications.delete({ where: { id: Number(id) } });
